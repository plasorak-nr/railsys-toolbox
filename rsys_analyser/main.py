...
def main(file:Path):
    ...
    logger.info(
        df
            .filter(pl.col('Station name') == 'Barnt Green')
            .group_by('Simulation no.')
            .agg(pl.count().alias('n_trains'))
    )
    logger.info(
        df
            .filter(pl.col('Station name') == 'Barnt Green')
    ...
    logger.info(
        df
            .filter(
                ...
                & (
                    (pl.col('Scheduled track') == 'BGRN T-2')
                    |
                    (
                        (pl.col('Scheduled track') != 'BGRN T-2')
                        &
            ...
            .select('Scheduled track', 'Actual track').unique()
        )
